...
export function enquiryInternalEmail(q) {
  return {
    subject: `New enquiry — ${q.name}${q.company ? ` · ${q.company}` : ''}`,
    html: wrap(
      'New enquiry',
      `<table role="presentation" cellpadding="0" cellspacing="0">
       ${row('Name', esc(q.name))}${row('Company', esc(q.company) || '—')}
       ${row('Email', `<a href="mailto:${esc(q.email)}">${esc(q.email)}</a>`)}
       ${row('Phone', `<a href="tel:${esc(q.phone)}">${esc(q.phone)}</a>`)}
       ${row('Interest', esc(q.topic) || '—')}
       </table>
       <p style="margin:18px 0 0;white-space:pre-wrap">${esc(q.message)}</p>`
    ),
  };
}

export function enquiryCustomerEmail(q) {
  return {
    subject: 'We have your message — DIV',
    html: wrap(
      'Thanks — we have your message',
      `<p style="margin:0 0 16px">Hi ${esc(q.name.split(' ')[0])}, an engineer will reply within one business day, usually sooner.</p>
       <p style="margin:0 0 16px;padding:14px 16px;background:#FBFAF8;border:1px solid #EDEAE3;border-radius:12px;white-space:pre-wrap">${esc(q.message)}</p>
       <p style="margin:0">If it is urgent, call or WhatsApp us on ${siteConfig.phone} — we answer 24/7.</p>`
    ),
  };
}
